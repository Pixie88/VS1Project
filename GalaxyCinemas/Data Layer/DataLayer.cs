...
namespace GalaxyCinemas.DataLayer
{
    public class DataLayer
    {
        ...
        /// </summary>
        public static List<Movie> GetAllMovies(int movieID, string Title)
        {
...
where [Sessions].SessionDate >= @startdate and [Session].SessionDate < @enddate";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
...
where SessionID = @sessionID";
        ...
        public static void AddMovie(Movie movie)
        {
            ...
            {
                conn.Open();
                string sql = @"insert into [Movies] (MovieID, Title)
        ...
        {
            {
                ...
                {
                    conn.Open();
                    string sql = @"insert into [Sessions] (SessionID, MovieID, SessionDate, CinemaNumber)
...
set MovieID=@movieID, SessionDate=@sessionDate, CinemaNumber=@cinemaNumber
where SessionID=@sessionID";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
                    command.Parameters.AddWithValue("sessionID", session.SessionID);
