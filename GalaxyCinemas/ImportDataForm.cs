    ...
    public partial class ImportDataForm : Form
    {
        ...
        private void btnSelectSessionFile_Click(object sender, EventArgs e)
        {
           ...
           if (result != System.Windows.Forms.DialogResult.OK)
            ...
            {
                TextReader tr = File.OpenText(opnFileDialog.FileName);
                txtbxSessionPath.Text = opnFileDialog.FileName;
                tr.Close();
            }
            catch (Exception)
            {
        ...
        private void btnImportSession_Click(object sender, EventArgs e)
        {
            ...
            // Do UI changes, e.g. hide Start button, show Stop button, start progress bar.
            btnSelectSessionFile.Visible = false;
            btnStopSession.Visible = true;
            progressBar.Value = 0;
            progressBar.Visible = true;

            // Start importer in another thread.
            sessionImporter = new SessionImporter(txtbxSessionPath.Text);
        ...
        private void txtSessionFileName_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            try
            {
                FileInfo fi = new FileInfo(txtbxSessionPath.Text);
                pathValid = fi.Directory.Exists;
            }
            catch (Exception)
            {
            ...
            if (!pathValid)
            {
                errorProvider.SetError(txtbxSessionPath, "Please choose a valid session file to import");
                e.Cancel = blockSubmitIfValidationFails; // Don't allow moving to the next field if we're submitting the form.
            }
            else errorProvider.SetError(txtbxSessionPath, ""); // Clear error if all fine.
        }
